...
with col1:
    team_a = st.selectbox("Select Team A", df_ligue1["Team"].unique(), index=0)
with col2:
    team_b = st.selectbox("Select Team B", df_ligue1["Team"].unique(), index=1)

# =========================
# Min-Max scaling
# =========================

scaled_df = pd.DataFrame(index=df_ligue1.index)

for col in all_metrics:
    col_min = df_ligue1[col].min()
    col_max = df_ligue1[col].max()
    if col_max - col_min == 0:
        # Zero-variance safeguard
        scaled_df[col] = 0.5
    else:
        scaled_df[col] = (df_ligue1[col] - col_min) / (col_max - col_min)
...
team_a_row = df_ligue1[df_ligue1["Team"] == team_a].iloc[0]
team_b_row = df_ligue1[df_ligue1["Team"] == team_b].iloc[0]

team_a_scaled = scaled_df.loc[df_ligue1["Team"] == team_a].iloc[0]
team_b_scaled = scaled_df.loc[df_ligue1["Team"] == team_b].iloc[0]
# =========================
# League average
# =========================
league_avg_real = df_ligue1[all_metrics].mean()
