...
df_bundesliga = load_data("Data/League Stat/Bundesliga.xlsx")
st.dataframe(df_bundesliga)
...
with col1:
    team_a = st.selectbox("Select Team A", df_bundesliga["Team"].unique(), index=0)
with col2:
    team_b = st.selectbox("Select Team B", df_bundesliga["Team"].unique(), index=1)

# =========================
# Min-Max scaling
# =========================

scaled_df = pd.DataFrame(index=df_bundesliga.index)

for col in all_metrics:
    col_min = df_bundesliga[col].min()
    col_max = df_bundesliga[col].max()
    if col_max - col_min == 0:
        # Zero-variance safeguard
        scaled_df[col] = 0.5
    else:
        scaled_df[col] = (df_bundesliga[col] - col_min) / (col_max - col_min)
...
team_a_row = df_bundesliga[df_bundesliga["Team"] == team_a].iloc[0]
team_b_row = df_bundesliga[df_bundesliga["Team"] == team_b].iloc[0]

team_a_scaled = scaled_df.loc[df_bundesliga["Team"] == team_a].iloc[0]
team_b_scaled = scaled_df.loc[df_bundesliga["Team"] == team_b].iloc[0]
# =========================
# League average
# =========================
league_avg_real = df_bundesliga[all_metrics].mean()
