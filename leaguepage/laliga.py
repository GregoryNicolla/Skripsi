...
df_laliga = load_data("Data/League Stat/Laliga.xlsx")
st.dataframe(df_laliga)
...
with col1:
    team_a = st.selectbox("Select Team A", df_laliga["Team"].unique(), index=0)
with col2:
    team_b = st.selectbox("Select Team B", df_laliga["Team"].unique(), index=1)

# =========================
# Min-Max scaling
# =========================

scaled_df = pd.DataFrame(index=df_laliga.index)

for col in all_metrics:
    col_min = df_laliga[col].min()
    col_max = df_laliga[col].max()

    if col_max - col_min == 0:
        # Zero-variance safeguard
        scaled_df[col] = 0.5
    else:
        scaled_df[col] = (df_laliga[col] - col_min) / (col_max - col_min)
...
team_a_row = df_laliga[df_laliga["Team"] == team_a].iloc[0]
team_b_row = df_laliga[df_laliga["Team"] == team_b].iloc[0]

team_a_scaled = scaled_df.loc[df_laliga["Team"] == team_a].iloc[0]
team_b_scaled = scaled_df.loc[df_laliga["Team"] == team_b].iloc[0]

# =========================
# League average
# =========================
league_avg_real = df_laliga[all_metrics].mean()
